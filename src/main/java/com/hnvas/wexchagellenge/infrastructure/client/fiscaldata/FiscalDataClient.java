...
public class FiscalDataClient extends RetryableApiClient {
  ...
  public FiscalDataClient(
      RetryTemplate retryTemplate, FiscalDataRestTemplateFactory restTemplateFactory) {
  ...
  public ExchangeRatesResponse getExchangeRatesByCountry(
      String country, LocalDate recordDateFrom, LocalDate recordDateTo) {
    var requestURI =
        endpointBuilder()
            .queryParam(
                FILTER_PARAM, FILTER_COUNTRY_VALUE.formatted(country, recordDateFrom, recordDateTo))
            .build()
            .toString();
    return getForObject(requestURI, ExchangeRatesResponse.class);
  }

  public ExchangeRatesResponse getExchangeRatesByCurrency(
      String currency, LocalDate recordDateFrom, LocalDate recordDateTo) {
    var requestURI =
        endpointBuilder()
            .queryParam(
                FILTER_PARAM,
                FILTER_CURRENCY_VALUE.formatted(currency, recordDateFrom, recordDateTo))
            .build()
            .toString();
    return getForObject(requestURI, ExchangeRatesResponse.class);
  }

  public ExchangeRatesResponse getExchangeRatesByCountryAndCurrency(
      String country, String currency, LocalDate recordDateFrom, LocalDate recordDateTo) {
    var requestURI =
        endpointBuilder()
            .queryParam(
                FILTER_PARAM,
                FILTER_COUNTRY_CURRENCY_VALUE.formatted(
